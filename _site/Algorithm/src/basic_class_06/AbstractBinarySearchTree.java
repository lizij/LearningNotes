...
public class AbstractBinarySearchTree {
		...
		if (deleteNode != null) {
			...
			if (deleteNode != null) {
				if (deleteNode.left == null) {
					// no left child, use right child
					nodeToReturn = transplant(deleteNode, deleteNode.right);
				} else if (deleteNode.right == null) {
					// no right child, use left child
					nodeToReturn = transplant(deleteNode, deleteNode.left);
				} else {
					// use the descendant node to replace deleteNode
					Node successorNode = getMinimum(deleteNode.right);
					if (successorNode.parent != deleteNode) {
						// replace descendant node with its right
	...
	private Node transplant(Node nodeToReplace, Node newNode) {
		//replace root node with newNode
		if (nodeToReplace.parent == null) {
			this.root = newNode;
		// set nodeToReplace's parent env
		} else if (nodeToReplace == nodeToReplace.parent.left) {
			nodeToReplace.parent.left = newNode;
		} else {
			nodeToReplace.parent.right = newNode;
		}
		// newNode's parent point to nodeToReplace.parent
		if (newNode != null) {
