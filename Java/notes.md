...
* 通过提供静态工厂方法如`newInstance`，避免通过构造器新建实例

* 在静态代码区初始化公共部分，保证类加载时已经初始化完成，减少重复创建消耗

* 提供适配器方案

  > 适配器是指把功能委托给一个后备对象，从而为后备对象提供一个可以替代的接口，也称为视图
  >
  > 例如Map接口的keySet方法返回Map的Set视图，包含所有key，对于给定的一个Map，每次keySet返回的是同一个实例，这个实例是可改变的，但是功能上没有区别，所以没有必要创建新的实例

* 如无必要，尽量不要手动维护自己的对象池，因为小对象的创建和回收是非常廉价的，除非是非常重量级的，例如数据库连接
...
> `newInstance`的问题
>
> * 会尝试调用类的无参构造器，而该构造器可能不存在，抛出InstantiationException
>
> * 没有可访问的构造器时不会在编译时抛出异常，而是在运行时抛出InstantiationException或IlegalAccessException
> * 会传播无参构造器抛出的任何异常
...
例如，假设可以构建泛型数组，反证法推导过程如下

```java
// (1) 创建一个类型为List<String>[]的泛型数组，容量为1
List<String>[] stringList = new ArrayList<>[1];
// (2) 创建一个List<Integer>，只包含一个元素 Integer(1)
List<Integer> intList = Arrays.asList(1);
// (3) 这行是合法的，因为stringList的类型是List<String>[]，擦除后是List[]，属于Object[]的子类型
Object[] objects = stringList
// (4) 这行是合法的，因为intList类型是List<Integer>，擦除后是List,属于Object的子类型
objects[0] = intList;
// (5) 这行是合法的，但是会抛出ClassCastException，出现矛盾，假设不成立
//     因为如果没有(4)，stringList[0].get(0)应返回String，编译器会增加自动强制转换
//     但是在(4)执行后，stringList[0]变为了intList，intList.get(0)返回类型为Integer，无法转换为String，所以会抛出ClassCastException
...
>1. 假设Sub是Super的子类型
>
>   数组是协变的，即Sub[]是Super[]的子类型
>
>   泛型是不变的，即List\<Sub>不是List\<Super>的子类型，擦除后都是原生类型List
>
>2. 数组在运行时才检查元素类型约束，会在运行时抛出异常
>
>   泛型通过擦除在编译时强化元素类型约束，在运行时则丢弃检查
...
2. 接口中的方法默认都是abstract类型，不能包含默认实现 
...
|    main方法    |               抽象类可以有main方法并且可以运行               |                  接口没有main方法，不能运行                  |
|     多继承     |              抽象类可以继承一个类和实现多个接口              |                 接口可以继承一个或者多个接口                 |
| 方法新增或修改 |             新增或修改方法不一定需要修改子类代码             |           新增或修改方法必须所有改变实现该接口的类           |

>接口提供了构造非层次结构的能力，可以安全地将不同层次的类组织到一个框架中，相比之下使用抽象类需要构造非常复杂的类层次
>
>抽象类的演变能力比接口的演变能力强，接口一旦公开发行并被广泛实现，再改变接口的成本很高
...
如果方法覆盖了超类中的一个方法，子类中的访问级别不允许低于超类中的访问级别，这样是为了保证可以使用超类实例的地方也可以使用子类实例，否则将在编译时产生错误信息

> 接口中定义的方法被隐式标记public权限，不能降为其他权限
...
public class Main {
    ...
    class B extends A {
        /**
         * Error:(13, 22) java: Main.B中的a()无法覆盖Main.A中的a()
         *   正在尝试分配更低的访问权限; 以前为public
         */
        private void funA() {super.funA();}

        /**
         * OK
         */
        @Override
        public void funB() {super.funB();}

        /**
         * OK
         */
        @Override
        protected void funC() {super.funC();}
    }
}
```
